...
def trace_len(trace: dict):
    ...
    return number_of_frames, frame_range_len, trace_lenn, max_step_len, max_step_len_step_index, max_step_len_line, max_step_len_frame_number
...
if __name__ == "__main__":
    ...
    with open('../data/Video_tracking/190822/20190822_112842909_2BEE_generated_20210503_074806_nn.csv', newline='') as csvfile:
         ...
         for index, trace in enumerate(traces.keys()):
            print()
            spam = trace_len(traces[trace])
            print("Agent nummber", index, ". Number_of_frames, frame_range_len, trace_lenn, max_step_len, max_step_len_index, max_step_len_line:", spam)
            if spam[2] == 0:
                print("This trace has length of 0. Consider deleting this agent")
            if spam[3] > bee_max_step_len:
