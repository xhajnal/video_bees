...
class MyTestCase(unittest.TestCase):
    ...
    def testTrimTrace(self):
        ...
        with open('../test/test.csv', newline='') as csv_file:
            ...
            trace0 = Trace(traces[0], 0)
            trace0.trim(1620, 1620)

            self.assertEqual(trace0.trace_id, 0)
            self.assertEqual(trace0.frame_range, [1621, 1622])
            self.assertEqual(trace0.get_number_of_frames_tracked(), 2)
            self.assertEqual(trace0.frame_range_len, 2)
            self.assertAlmostEqual(trace0.trace_length, 2.8284271247461903)
            self.assertAlmostEqual(trace0.max_step_len, 2.8284271247461903)
            self.assertEqual(trace0.max_step_len_step_index, 0)
            self.assertEqual(trace0.max_step_len_line, 3)
            self.assertEqual(trace0.max_step_len_frame_number, 1621)
            self.assertEqual(trace0.trace_lengths, {2.828427: 1})
            self.assertEqual(trace0.frames_list, [1621, 1622])
            self.assertEqual(trace0.locations, [[1.0, 1.0], [3.0, 3.0]])
            self.assertEqual(trace0.get_frame_list(), [1621, 1622])
            ...
            self.assertEqual(trace0.get_location_from_frame(1621), [1.0, 1.0])
            self.assertEqual(trace0.get_location_from_frame(1622), [3.0, 3.0])
            self.assertEqual(trace0.get_locations_from_frame_range((1621, 1622)), [[1.0, 1.0], [3.0, 3.0]])
            self.assertEqual(trace0.get_number_of_frames_tracked(), 2)
            trace0.assert_trace_consistency()
            self.assertEqual(trace0.check_whether_is_done([1620, 1635]), False)
            self.assertEqual(trace0.check_whether_is_done([1621, 1622]), True)
            self.assertAlmostEqual(trace0.trace_length, 2.8284271247461903)
            self.assertEqual(trace0.trace_lengths, {2.828427: 1})
            self.assertAlmostEqual(trace0.max_step_len, 2.8284271247461903)
